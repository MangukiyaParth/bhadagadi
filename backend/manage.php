...
$login_not_require_operation = array("get_version_data", "login_user", "add_user", "get_state", "get_city", "get_all_basics", "get_car_type", "get_plans", "user_verification");
$plan_not_require_operation = array("get_version_data", "login_user", "add_user", "get_state", "get_city", "get_all_basics", "get_car_type", "get_plans", "user_verification", "add_user_plan", "get_banners");
...
if (empty($json_feed)) {
	if (empty($temp_outputjson['data'])) {
		unset($temp_outputjson['data']);
	}
	$response_string = json_encode(($temp_outputjson), JSON_INVALID_UTF8_IGNORE | JSON_PRETTY_PRINT);
} else if (!empty($temp_outputjson['data'])) {
...
function getUsersDetails($id, $is_md5)
{
	...
	if ($is_md5) {
		$query_user = "SELECT usr.*, 
				...
				(SELECT STR_TO_DATE(end_date,'%Y-%m-%d %H:%i:%s') FROM `tbl_users_plan` WHERE user_id = usr.id AND STR_TO_DATE('$current_date','%Y-%m-%d %H:%i:%s') BETWEEN STR_TO_DATE(start_date,'%Y-%m-%d %H:%i:%s') AND STR_TO_DATE(end_date,'%Y-%m-%d %H:%i:%s') LIMIT 1) AS active_plan_exp_date,
				(SELECT COUNT(id) FROM `tbl_vehicle_requirement` WHERE created_by = usr.id ) AS total_trip
